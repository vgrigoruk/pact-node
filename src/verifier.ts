...
export class Verifier {
	...
	constructor(options: VerifierOptions) {
		...
		if (options.pactBrokerUrl) {
			logger.warn("'pactBrokerUrl' is deprecated, please use 'pactBrokerBaseUrl' instead");
		}

		if (checkTypes.emptyArray(options.pactUrls) && !options.pactBrokerBaseUrl && !options.pactBrokerUrl) {
			throw new Error("Must provide the pactUrls argument if no pactBrokerBaseUrl provided");
		}

		if (((!options.pactBrokerBaseUrl && !options.pactBrokerUrl) || _.isEmpty(options.provider)) && checkTypes.emptyArray(options.pactUrls)) {
			throw new Error("Must provide both provider and pactBrokerBaseUrl if pactUrls not provided.");
		}
